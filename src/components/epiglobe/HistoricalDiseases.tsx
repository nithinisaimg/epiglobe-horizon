...
import { getHistoricalDiseases, spreadRateLabel, spreadRateColor } from '@/utils/historicalDiseases';
          ...
          {diseases.map((d, i) => {
            ...
            return (
              <div
                key={`${d.name}-${i}`}
                className="p-3"
                style={{ background: '#0D1525', border: '1px solid #1A2540', borderLeft: `2px solid ${color}` }}
              >
                <div className="flex items-start justify-between gap-2">
                  <div className="text-xs font-semibold" style={{ color: '#E8EDF5' }}>{d.name}</div>
                  <div className="text-[10px] tracking-[0.1em] uppercase shrink-0" style={{ color: '#E8EDF566' }}>{d.year}</div>
                </div>
                <div className="mt-2 grid grid-cols-2 gap-1 text-[10px]" style={{ color: '#E8EDF599' }}>
                  <div>Cases: <span style={{ color: '#E8EDF5' }}>{d.cases.toLocaleString()}</span></div>
                  <div>Deaths: <span style={{ color: '#FF3B3B' }}>{d.deaths.toLocaleString()}</span></div>
                </div>
