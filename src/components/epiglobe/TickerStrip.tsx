...
import { DISEASES, synthesizeCountryData, severityColorFor } from "@/utils/diseaseRegistry";
...
export default function TickerStrip({ data }: TickerStripProps) {
  ...
  const items = useMemo(() => {
    ...
    const perDisease = nonCovid.map((d) => ({
      disease: d,
      byCountry: new Map(synthesizeCountryData(data, d).map((c) => [c.country, c])),
    }));
  ...
  const content = items.map((item, i) => (
    <span
      key={i}
      className="inline-flex items-center gap-2 mx-4 whitespace-nowrap text-xs tracking-wide"
    >
