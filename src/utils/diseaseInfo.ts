...
export const DISEASE_INFO: Record<string, DiseaseInfo> = {
  covid19: {
    ...
    prediction:
      "Transitioning to endemic seasonal pattern. Expect winter waves driven by new Omicron sub-variants; severity continues to decline with population immunity.",
  },
  influenza: {
    ...
    prediction:
      "Annual seasonal cycles will continue. Risk of a novel pandemic strain (e.g. H5N1 spillover) remains moderate; surveillance and stockpiled vaccines reduce impact.",
  },
  mpox: {
    ...
    prediction:
      "Sporadic outbreaks expected to continue in non-endemic regions. Clade I (more severe) spread in Central Africa is the main concern for 2025-26.",
  },
  dengue: {
    ...
    prediction:
      "Cases projected to rise sharply with climate change expanding Aedes mosquito range. Expected doubling of at-risk population by 2050.",
  },
  malaria: {
    ...
    prediction:
      "Slow decline possible with vaccine rollout, but drug-resistant P. falciparum and insecticide resistance threaten progress. Eradication unlikely before 2050.",
  },
  ebola: {
    ...
    prediction:
      "Sporadic Central/West African outbreaks will continue due to animal reservoirs (fruit bats). Fast-response vaccination keeps outbreaks small.",
  },
  sars: {
    ...
    prediction:
      "Re-emergence risk is low but non-zero. Any new coronavirus spillover would likely follow SARS-CoV-2 mRNA platform response.",
  },
  h1n1: {
    ...
    prediction:
      "Now endemic as seasonal flu. No pandemic-level resurgence expected from this specific strain.",
  },
  spanish_flu: {
    ...
    prediction:
      "Strain is extinct as a distinct entity. Modern descendants circulate as seasonal H1N1; pandemic flu risk persists but response capacity is vastly improved.",
  },
  plague: {
    ...
    prediction:
      "Small endemic foci persist (Madagascar, western USA, Mongolia). Global pandemic risk is minimal with modern sanitation and antibiotics.",
  },
  smallpox: {
    ...
    prediction:
      "No natural circulation. Only risk is accidental lab release or bioterrorism; global ring-vaccination plan is in place.",
  },
};

export function getDiseaseInfo(id: string): DiseaseInfo {
  return (
    DISEASE_INFO[id] ?? { cure: "Information unavailable.", prediction: "Information unavailable." }
  );
}
